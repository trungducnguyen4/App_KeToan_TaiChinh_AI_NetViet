...
const bankDebitColumns: BankDebitColumn[] = [
...
export default function BankDebitListScreen() {
  const bankDebitVouchers = useMemo(() => cashVouchers.filter((voucher) => voucher.voucherType === "BN"), []);
  const [selectedId, setSelectedId] = useState(bankDebitVouchers[0]?.id ?? "");
  ...
  const filteredVouchers = useMemo(() => {
    ...
    return bankDebitVouchers.filter((voucher) => {
      if (!normalized) {
        return true;
      }
  ...
  }, [bankDebitVouchers, query]);

  const selectedVoucher = filteredVouchers.find((voucher) => voucher.id === selectedId) ?? filteredVouchers[0] ?? bankDebitVouchers[0];
    ...
    <AppShell activeModule="cash">
      <div className="workspace">
        <div className="breadcrumb">
          ...
          <span>/</span>
          <span>Báo nợ ngân hàng</span>
        </div>

        <section className="hero-panel">
          <div>
            <div className="eyebrow">Màn danh sách</div>
            <h2 className="hero-title">Báo nợ ngân hàng</h2>
            <p className="hero-copy">
              Danh sách chứng từ BN dùng chung template bảng, có đủ metadata và chi tiết hạch toán phía dưới.
            </p>
          </div>
        ...
        <div className="section-title">
          ...
          <div className="topbar-actions">
            <a className="button primary" href="/modules/cash/bank-debits/new">
